...
Также в C# есть такая функция как **Exception Filtering**, которая позволяет фильтровать исключения по определенным условиям. Это позволяет вам обрабатывать только те исключения, которые соответствуют заданным критериям. Например:
...
{
    // Обработка исключений ArgumentException и InvalidOperationException
    Console.WriteLine(ex.Message);
}
```
или же 
...
catch (Exception ex) when (ex.Message.Contains("ошибка"))
{
    // Обработка исключений, содержащих слово "ошибка" в сообщении
    Console.WriteLine(ex.Message);
}
```
